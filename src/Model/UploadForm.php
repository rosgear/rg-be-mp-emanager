...
class UploadForm extends FormModel
{
     ...
     * @param array|null $attributes Имена атрибутов с их значениями, если не указаны - будут 
    ...
    protected function unpackProcess(): bool
    {
        ...
        $params = Ge::$app->extensions->getRegistry()->get($package->id);
        if ($params) {
            /** @var array|null $names */
            $names = Ge::$app->extensions->selectName($params['rowId']);
            $this->setError(
                $this->module->t(
